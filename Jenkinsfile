pipeline {
  agent {
    ...
    environment {
        ...
        REGISTRY = 'docker.io'
        DOCKERHUB_NAMESPACE = 'lovekernel'
        GITHUB_ACCOUNT = 'loveresolution'
              ...
              container ('maven') {
                input(id: 'release-image-with-tag', message: 'release image with tag?')
                  withCredentials([usernamePassword(credentialsId: "$GITHUB_CREDENTIAL_ID", passwordVariable: 'GIT_PASSWORD', usernameVariable: 'GIT_USERNAME')]) {
                    sh 'git config --global user.email "zhfeijian@gmail.com" '
                    sh 'git config --global user.name "loveresolution" '
